...
public class GLApp {
	...
	public void loadSettings(String configFilename) {
		if ((configFilename == null) || configFilename.equals("")) { return; }
	...
	public static DisplayMode getDisplayMode(int w, int h, int colorBits, int freq) {
		try {
			...
			for (int j = 0; j < allDisplayModes.length; j++) {
				...
				if ((dm.getWidth() == w) && (dm.getHeight() == h) && (dm.getBitsPerPixel() == colorBits) && (dm.getFrequency() == freq)) { return dm; }
			}
	...
	*/
	...
	public static void updateTimer() {
		...
		secondsSinceLastFrame = (Sys.getTime() - lastFrameTime) / ticksPerSecond;
	...
	public static GLImage loadImage(String imgFilename) {
		GLImage img = new GLImage("res/" + imgFilename);
	...
	public static void drawImageFullScreen(GLImage img) {
		if ((img == null) || (img.isLoaded() == false)) { return; }
	...
	public static void renderPlane(float length, float height, int length_segments, int height_segments, float tilefactorU, float tilefactorV) {
		...
		float segsizeL = length / length_segments;
		float segsizeH = height / height_segments;
		float maxDimension = (length > height) ? length : height;
		float uvsegsizeL = (length / maxDimension) / length_segments;
		float uvsegsizeH = (height / maxDimension) / height_segments;
		GL11.glBegin(GL11.GL_QUADS);
		{
	...
	public static boolean buildFont(String charSetImage, int fontWidth) {
		...
		if (textureImg == null) { return false; // image not found
		}
		//pushAttrib();
	...
	public static void buildFont(int fontTxtrHandle, int fontWidth) {
		...
		for (int i = 0; i < 256; i++) {
			cx = (i % 16) / 16f; // X Texture Coord Of Character (0 - 1.0)
			cy = i / 16 / 16f; // Y Texture Coord Of Character (0 - 1.0)
	...
	public static String makeTimestamp() {
		...
		String datetime = "" + year + (month < 10 ? "0" : "") + month + (day < 10 ? "0" : "") + day + "-" + (hours < 10 ? "0" : "") + hours + (minutes < 10 ? "0" : "") + minutes + (seconds < 10 ? "0" : "") + seconds;
		return datetime;
	}
