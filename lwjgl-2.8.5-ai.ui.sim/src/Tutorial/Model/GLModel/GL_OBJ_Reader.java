package Tutorial.Model.GLModel;
...
public class GL_OBJ_Reader {
	...
	public void loadobject(BufferedReader br) {
		...
		// debug:
		System.out.println("GL_OBJ_Reader: read " + numpolygons() + " faces in " + groups.size() + " groups");
		// debug:
	...
	private float[] read3Floats(String line) {
		try {
			...
			if (st.countTokens() == 2) { // texture uv may have only 2 values
				return new float[] { Float.parseFloat(st.nextToken()), Float.parseFloat(st.nextToken()), 0 };
			} else {
	...
	public Group findGroup(String name) {
		for (int i = 0; i < groups.size(); i++) {
			if (((Group) groups.get(i)).name.equals(name)) { return (Group) groups.get(i); }
		}
		return null;
	}
