...
public class EightPuzzleBoard {
	...
	public int getMissplaceTile(int[] state) {
		int count = 0;
	...
	public int getMahattan(EightPuzzleBoard board) {
		int count = 0;
		for (int i = 1; i <= 9; i++) {
			int atx = 0;
			XYLocation l1 = getLocationA(i);
			XYLocation l2 = board.getLocationA(i);

			int deltaX = l1.getXCoOrdinate() - l2.getXCoOrdinate();
			int deltaY = l1.getYCoOrdinate() - l2.getYCoOrdinate();

			atx = Math.abs(deltaX) + Math.abs(deltaY);
			count += atx;
			System.out.println("i " + i + ": " + atx);
		}
		return count;
	}
	...
	private XYLocation getLocationA(int i) {
	...
	public boolean equals(Object o) {
		...
		for (int i = 0; i < 8; i++) {
			if (getPositionOf(i) != aBoard.getPositionOf(i)) { return false; }
		}
		return true;
	}
