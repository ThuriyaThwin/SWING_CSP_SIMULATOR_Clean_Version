...
public class NQueensBoard {
	...
	public int getNumberOfAttacksWhenMoveMove(XYLocation from, XYLocation to) {
		NQueensBoard nQueensBoard = new NQueensBoard(size);
		List<XYLocation> xy = getQueenPositions();
		for (XYLocation xyLocation : xy) {
			nQueensBoard.addQueenAt(xyLocation);
		}
		nQueensBoard.removeQueenFrom(from);
		nQueensBoard.addQueenAt(to);
		return nQueensBoard.getNumberOfAttackingPairs();
	}
	...
	public XYLocation getFirstNQueenInXLine(int i) {
		for (int j = 0; j < size; j++) {
			XYLocation xy = new XYLocation(i, j);
			if (queenExistsAt(xy)) { return xy; }
		}
		return null;
	}
}